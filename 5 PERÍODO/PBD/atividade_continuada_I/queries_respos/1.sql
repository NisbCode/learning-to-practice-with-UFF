...
Create index idx_lineitem on lineitem (l_returnflag, l_linestatus);

EXPLAIN select idx_lineitem, sum(l_quantity) as sum_qty, sum(l_extendedprice) 
as sum_base_price, sum(l_extendedprice * (1 - l_discount)) 
as sum_disc_price, sum(l_extendedprice * (1 - l_discount) * (1 + l_tax)) 
as sum_charge, avg(l_quantity) as avg_qty, avg(l_extendedprice) 
as avg_price, avg(l_discount) 
...
from LINEITEM 
where l_shipdate <= date '1998-12-01' - interval '108' day 
group by idx_lineitem 
order by idx_lineitem;